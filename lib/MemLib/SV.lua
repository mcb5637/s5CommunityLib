...
if mcbPacker then
    ...
    mcbPacker.require("s5CommunityLib/Lib/MemLib/Offsets")
	...
	MemLib.Load("LAU", "Offsets", "Comfort/Number/IntFloatConversion")
end
--------------------------------------------------------------------------------
MemLib.SV = {}
...
if XNetwork.Manager_IsNATReady then
    ...
    function MemLib_SV_InitAddressEntity()
        ...
        function MemLib_SV_InitAddressEntityJob(_X, _Y)
            local entityAddress = Logic.GetEntityScriptingValue(MemLib.SV.AddressEntity, MemLib.Offsets.Settler.SelfAddress)
            if entityAddress < 1 then
                ...
                MemLib.SV.AddressOffset = MemLib.LAU.ToNumber(entityLargeAddress + MemLib.Offsets.Entity.ScriptingValue * 4)
    ...
    function MemLib.SV.AddressGetInt(_Address)
        ...
        local offset = MemLib.LAU.ToTable(_Address) - MemLib.SV.AddressOffset
        local byteOffset = math.mod(MemLib.LAU.ToNumber(offset), 4)
        local alignedOffset = math.floor(MemLib.LAU.ToNumber(MemLib.LAU.DivTable4_Inplace(offset)))

        if byteOffset == 0 then
            return Logic.GetEntityScriptingValue(MemLib.SV.AddressEntity, alignedOffset)
        else
            local alignedOffset1 = alignedOffset
            local alignedOffset2 = MemLib.LAU.ToNumber(MemLib.LAU.ToTable(alignedOffset1) + 1)
            ...
            return MemLib.LAU.ToNumber(MemLib.LAU.Or(value1, value2))
        end
    end
    --------------------------------------------------------------------------------
    ---@param _Address integer
    ...
    function MemLib.SV.AddressSetInt(_Address, _Value)
        ...
        if byteOffset == 0 then
            ...
            Logic.SetEntityScriptingValue(MemLib.SV.AddressEntity, alignedOffset1, MemLib.LAU.ToNumber(value1))
            Logic.SetEntityScriptingValue(MemLib.SV.AddressEntity, alignedOffset2, MemLib.LAU.ToNumber(value2))
        end
    end
    --------------------------------------------------------------------------------
    ---@param _Address integer
    ...
    function MemLib.SV.AddressGetByte(_Address)
        ...
        return MemLib.LAU.ToNumber(MemLib.LAU.And(value, MemLib.SV.FF))
    end
    --------------------------------------------------------------------------------
    ---@param _Address integer
    ---@param _Value integer
    function MemLib.SV.AddressSetByte(_Address, _Value)
        ...
        Logic.SetEntityScriptingValue(MemLib.SV.AddressEntity, alignedOffset, MemLib.LAU.ToNumber(value))
    end
    --------------------------------------------------------------------------------
    -- setup userdata
    --------------------------------------------------------------------------------
    ...
    function MemLib.SV.mt.__index(_Table, _Key)
        if type(_Key) == "number" then
            ...
            address = MemLib.LAU.ToNumber(MemLib.LAU.ToTable(address) + _Key * 4)
    ...
    function MemLib.SV.New(_Value, _Address)
        ...
        function userdata:GetByte(_Offset)
            ...
            local value
            if self.Address then
                value = MemLib.SV.AddressGetByte(MemLib.LAU.ToNumber(MemLib.LAU.ToTable(self.Address) + _Offset))
            else
                value = MemLib.LAU.ToNumber(MemLib.LAU.ToTable(self.Value) + _Offset)
            end
        ...
        function userdata:SetByte(_Offset, _Value)
            if self.Address then
                _Offset = _Offset or 0
                MemLib.SV.AddressSetByte(MemLib.LAU.ToNumber(MemLib.LAU.ToTable(self.Address) + _Offset), _Value)
            end
        end

        function userdata:Offset(_Offset)
            local value = MemLib.LAU.ToNumber(MemLib.LAU.ToTable((self.Address or self.Value)) + _Offset * 4)
